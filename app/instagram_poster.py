...
# FIX 2026-04-28e: kill switch global — INSTAGRAM_DISABLED=1 deshabilita TODO IG.
# Activado tras incidente IG bloqueado + 3 envios duplicados. El usuario controla.
_IG_KILL = os.getenv("INSTAGRAM_DISABLED", "").strip().lower() in ("1","true","yes","on")
IG_ENABLED = (not _IG_KILL) and bool(IG_USERNAME and IG_PASSWORD)
if _IG_KILL:
    log.warning("📸 Instagram DESHABILITADO globalmente (INSTAGRAM_DISABLED=1)")
    ...
    try:
        ...
        if IG_SESSION_FILE.exists():
            try:
                ...
                # FIX 2026-04-28: HEALTH CHECK pre-uso — validar sesion con
                # account_info() barato (1 request). Si falla, no devolvemos
                # cliente roto que crashee photo_upload despues.
                try:
                    cl.account_info()
                    cl.dump_settings(IG_SESSION_FILE)  # refresh cookies si OK
                    _ig_client = cl
                    log.info("Instagram: sesion restaurada y validada OK")
                    return cl
                except Exception as _ev:
                    log.warning(f"Instagram: sesion cargada pero account_info fallo ({_ev}) — descartando")
                    # Continua a login fresco abajo
            except Exception:
                log.debug("Sesion guardada expirada, haciendo login fresco")

        # Login fresco
        cl.login(IG_USERNAME, IG_PASSWORD)
        # FIX 2026-04-28: validar tras login fresco antes de cachear
        try:
            cl.account_info()
        except Exception as _ev2:
            log.warning(f"Instagram: login fresco hecho pero account_info fallo ({_ev2}) — challenge probable")
            _notify_ig_failure(f"login fresco fallido (account_info: {_ev2})")
            return None
        cl.dump_settings(IG_SESSION_FILE)
        _ig_client = cl
        log.info("Instagram: login exitoso y validado")
        return cl

    except Exception as e:
        log.warning(f"Instagram login fallido: {e}")
        _notify_ig_failure(f"login excepcion: {e}")
        return None


# FIX 2026-05-18 P2.14: alerta al admin si IG se cae.
# Antes los fallos eran silenciosos en log y nadie se enteraba hasta revisar
# manualmente. Rate limit 6h para no spamear.
_last_ig_admin_alert = 0.0

def _notify_ig_failure(reason: str) -> None:
    """Best-effort: notifica al admin (USER_ID_1) cuando Instagram cae.
    Limitado a 1 aviso cada 6h para evitar spam si el error persiste."""
    global _last_ig_admin_alert
    try:
        now = time.time()
        if now - _last_ig_admin_alert < 21600:  # 6h
            return
        ...
        msg = (f"⚠️ *Instagram caido*\n"
               f"Razon: `{str(reason)[:140]}`\n"
               f"Las celebraciones IG no se publican hasta que se restaure el login "
               f"(probable 2FA o challenge — revisar manualmente).")
        _rq.post(
            f"https://api.telegram.org/bot{bot_token}/sendMessage",
            json={"chat_id": admin_id, "text": msg, "parse_mode": "Markdown"},
            timeout=8,
        )
        _last_ig_admin_alert = now
    except Exception:
        pass
...
# FIX 2026-04-28: quotes/tips/market hours traducidos al INGLES
_MOTIVATIONAL_QUOTES = [
    ("Discipline is the bridge\nbetween goals and accomplishment.", "Jim Rohn"),
    ("The market rewards patience,\nnot rush.", "BuySell365"),
    ("A good trader doesn't predict,\nmanages risk.", "BuySell365"),
    ("Consistency beats talent\nwithout discipline.", "BuySell365"),
    ("Protect your capital.\nOpportunities always return.", "BuySell365"),
    ("90% of trading is\npsychology and risk management.", "BuySell365"),
    ("Don't trade by emotion.\nTrade by strategy.", "BuySell365"),
    ("Losses are the cost\nof doing business in the market.", "BuySell365"),
    ("A trading plan without discipline\nis just a wish list.", "BuySell365"),
    ("The best trade is the one\nyou don't take when there's no setup.", "BuySell365"),
    ("Lose small, win big.\nThat's how you build an account.", "BuySell365"),
    ("You don't need to always win.\nYou need to win more than you lose.", "BuySell365"),
]

_TRADING_TIPS = [
    ("Tip: Stop Loss", "NEVER trade without Stop Loss.\nIt's your life insurance in the market.\nDefine your risk BEFORE entering."),
    ("Tip: Risk Management", "Never risk more than 1-2%\nof your account per trade.\nSurvive > win."),
    ("Tip: Key Hours", "Best opportunities:\n08:00-11:00 (London)\n14:30-17:00 (NY overlap)\nAvoid Asian session if you trade Forex."),
    ("Tip: Trading Journal", "Record EVERY single trade.\nWithout data there's no improvement.\nReview your journal weekly."),
    ("Tip: Overtrading", "More trades \u2260 more profit.\nQuality > Quantity.\n3 good trades > 15 mediocre."),
    ("Tip: News Events", "Don't trade 30 min before/after\nhigh-impact news (NFP, CPI, FOMC).\nSpread explodes."),
    ("Tip: The Trend", "The trend is your friend.\nDon't fight the market.\nTrade in the direction of momentum."),
    ("Tip: Take Profit", "Don't be greedy.\nTake partials when the market gives.\nLocking in profits = consistency."),
]

_MARKET_HOURS = {
    "Sydney":   ("22:00", "07:00", "AUD, NZD"),
    "Tokyo":    ("00:00", "09:00", "JPY, Asian pairs"),
    "London":   ("08:00", "17:00", "GBP, EUR — Highest volume"),
    "New York": ("13:00", "22:00", "USD — Overlap with London"),
}
        ...
        for frame_i in range(total_frames):
            ...
            # ── ESCENA 3: CTA (6.5-8s) — expandido ──
                ...
                if scene_t > 0.3:
                    ...
                    draw.text((REEL_W // 2, 845), "JOIN VIP CHANNEL", fill=(10, 10, 16),
...
def post_daily_summary(stats: dict) -> bool:
    ...
    # FIX 2026-04-28: jitter humano 0-15 min antes de publicar daily summary para
    # romper el patron temporal predecible (12:00/19:00 exactos) que IG detecta
    # como bot-like. Reduce probabilidad de challenge update_risky_contactpoint.
    import random as _rnd_jitter
    _jitter_secs = _rnd_jitter.randint(0, 900)
    if _jitter_secs > 0:
        log.info(f"IG human jitter: esperando {_jitter_secs}s antes de publicar daily summary")
...
def post_scheduled_content(content_type: str = "auto") -> bool:
    ...
    # FIX 2026-04-28: jitter humano 0-15 min para romper patron temporal bot-like
    import random as _rnd_jitter_sched
    _jitter_secs = _rnd_jitter_sched.randint(0, 900)
    if _jitter_secs > 0:
        log.info(f"IG human jitter: esperando {_jitter_secs}s antes de scheduled content")
        time.sleep(_jitter_secs)
