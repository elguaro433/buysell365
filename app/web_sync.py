...
# FIX 2026-04-28: retirado fallback hardcoded "buysell365_sync_2026" — si SYNC_SECRET
# no esta en env, log warning y deshabilita sync (no envia con secret expuesto).
SYNC_SECRET = os.getenv("SYNC_SECRET", "").strip()
if not SYNC_SECRET:
    logger.warning("SYNC_SECRET no configurado — web_sync queda DESHABILITADO. Configura env var para activar sync.")
...
def _do_sync(get_state_fn, on_signals_fn):
    ...
    """
    # FIX 2026-04-28: si SYNC_SECRET no esta configurado, retornar False sin enviar
    # (evita exponer estado a terceros con secret hardcoded). Silenciosamente skip.
    if not SYNC_SECRET:
        return False
    import requests
    try:
